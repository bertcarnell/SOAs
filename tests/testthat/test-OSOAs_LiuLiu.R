test_that("OSOAs_LiuLiu", {
  ...
  temp <- OSOAs_LiuLiu(DoE.base::L16.4.5, t = 2, m = 3)
  ...
  expect_error(OSOAs_LiuLiu(FrF2::FrF2(32,6), t = 3, m = 3), regexp="m <= boundm", fixed=TRUE)
  expect_error(OSOA_LiuLiut(DoE.base::L16.4.5, t = 4, m = 4))

  ## check 3-level
  temp <- OSOAs_LiuLiu(lhs::createBusht(3, 4, 4))
  expect_equal(attr(temp, "strength"), 4)
  expect_equal(attr(temp, "type"), "OSOA")
