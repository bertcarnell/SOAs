...
#'
#' @return
#' \code{contr.FFsym} yields a matrix of real-valued contrasts.
...
contr.FFsym <- function(n, s, contrasts=TRUE, slowfirst=TRUE){
  if (!contrasts)
    stop("contr.FFsym not defined for contrasts=FALSE")
  ...
  if (!s^round(log(lenglev, base=s)) == lenglev)
    stop("contr.FFsym requires that the number of levels is a power of s.")
